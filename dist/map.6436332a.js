...
},{"d3-collection":"../node_modules/d3-collection/src/index.js","d3-selection":"../node_modules/d3-selection/src/index.js"}],"data/data_NY.json":[function(require,module,exports) {
module.exports="/data_NY.a1adb8c3.json";
},{}],"data/Testing_Sites.csv":[function(require,module,exports) {
module.exports = "/Testing_Sites.076f3f6b.csv";
},{}],"scripts/map.js":[function(require,module,exports) {
"use strict";
...
var tipHeight = 50 - margin.top - margin.bottom;
var tipWidth = 75 - margin.left - margin.right;
var svg = d3.select('#chart-change').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
var parseTime = d3.timeParse('%Y'); // const projection = d3.geoMercator().translate([width / 2, height / 2])
// const path = d3.geoPath().projection(projection)
// below is for the tooltip

var xTooltip = d3.scaleLinear().domain([parseTime('2009'), parseTime('2019')]).range([0, tipWidth]);
var yTooltip = d3.scaleLinear().domain([0, 100]).range([tipHeight, 0]);
var line = d3.line().curve(d3.curveCardinal).x(function (d) {
  return xTooltip(+d.datetime);
}).y(function (d) {
  return yTooltip(+d.percent_recycled);
});
var colorScale = d3.scaleOrdinal().domain([0, 40]).range(['#f2f0f7', '#cbc9e2', '#9e9ac8', '#756bb1', '#54278f']);
Promise.all([d3.json(require('/data/data_NY.json')), d3.csv(require('/data/Testing_Sites.csv'))]).then(ready).catch(function (err) {
...
function ready(_ref) {
  ...
  var hood = topojson.feature(datapoints, datapoints.objects.data_NY); // console.log(districts)

  console.log(datapoints);
  console.log(siteData);
  var projection = d3.geoConicConformal().parallels([40 + 40 / 60, 41 + 2 / 60]).rotate([74, 0]) // .translate([width / 2, height / 2])
  .fitSize([width, height], hood);
  var path = d3.geoPath().projection(projection); // colorScaleNegative.domain([0, d3.min(pctPtChange)])
  // ----Draw tip here------

  var formatComma = d3.format(',');
  var tip = d3.tip().attr('class', 'd3-tip').style('position', 'absolute').offset([10, 10]).html(function (d) {
    return "<b>".concat(d.properties.NEIGHBORHOOD_NAME, "</b> <br>  <b>Pct Tested:</b> <span style='color:red'>").concat(formatComma(d.properties.test_pct), "</span>\n    <br> <span style='color:none'>").concat(formatComma(d.properties.median_household_income), "</span>");
  });
  svg.call(tip); // ----- TIP ENDS HERE-------
  // -----DRAW MAP HERE------

  svg.selectAll('.hood').data(hood.features).enter().append('path').attr('class', 'hood').attr('d', path).attr('stroke', 'none').attr('fill', function (d) {
    return colorScale(+d.properties.PERCENT_POSITIVE);
  }).on('mouseenter', function () {
    d3.select(this).raise().transition().style('transform', 'scale(1,1)');
  }).on('mouseleave', function () {
    d3.select(this).lower().transition().style('transform', 'scale(1,1)');
  }).on('mouseover', tip.show).on('mouseout', tip.hide); // ----STEP FUNCTIONS START HERE------SHOWs TEST PERCENTAGE

  d3.select('#step1').on('stepin', function () {
    console.log('step1');
    svg.selectAll('path').attr('stroke', 'none').attr('fill', function (d) {
  ...
  d3.select('#step3').on('stepin', function () {
    console.log('step3');
    svg.selectAll('path').attr('stroke', function (d) {
      if ((+d.properties.pct_hispanic_latino > 0.25) + (+d.properties.pct_black > 0.25)) {
        console.log((+d.properties.pct_hispanic_latino > 0.25) + (+d.properties.pct_black > 0.25)); // console.log(+d.properties.pct_black)
  ...
  d3.select('#step4').on('stepin', function () {
    console.log('step4');
    svg.selectAll('.circle').data(siteData).enter().append('circle').attr('class', 'circle').attr('r', 4).attr('opacity', 0.9).attr('transform', function (d) {
      var coords = [+d.longitude, +d.latitude];
      return "translate(".concat(projection(coords), ")");
    }).attr('fill', 'green').attr('stroke', 'none');
...
// render()
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","d3-tip":"../node_modules/d3-tip/index.js","/data/data_NY.json":"data/data_NY.json","/data/Testing_Sites.csv":"data/Testing_Sites.csv"}]},{},["scripts/map.js"], null)
//# sourceMappingURL=/map.6436332a.js.map